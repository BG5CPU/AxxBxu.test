...
for it = 1:sample_s
    Ax = [ 1, -ph; ph, 1-ph*pe+ph*pe*Xs(1,it)^2 ];
...
Px1 = x1 - ph*x2 + ph*(1-0.45*x2.^2+0.45*exp(x1)).*0 - x1;
...
cvx_begin sdp % quiet
    ...
    minimize( mu_u - 0*mu_l + 1*epGa - 0.1*trace(epGa) );
...
yleft = -xrr-ybis; yright = xrr-ybis;
[x1, x2] = meshgrid(xleft:dx:xright, yleft:dx:yright);
Px1 = x1 - ph*x2 + ph*(1-0.45*x2.^2+0.45*exp(x1)).*saturU(vK(1)*x1+vK(2)*x2,barU) - x1;
