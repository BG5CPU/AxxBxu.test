...
rx = 1.18;
rho11   = 1;                           % 1
rho12_u = rx^2;    rho12_l = 0;        % x2^2
rho13_u = exp(rx); rho13_l = exp(-rx); % exp(x1)

Vrho = { [rho11, rho12_u, rho13_u]';
         [rho11, rho12_u, rho13_l]'; 
         [rho11, rho12_l, rho13_u]'; 
         [rho11, rho12_l, rho13_l]' };
...
cvx_begin sdp % quiet
    ...
    subject to   
        for iv = 1:length(Vrho)
            EQX = Ed*Vrho{iv};
            blockS = [ -lyGa,                  Ad*lyGa + EQX*kY;
                        lyGa*Ad' + kY'*EQX',  -lyGa + epGa*eye(dimN) ];
            blockS <= 0;
        end
        epGa >= 0.001;
        lyGa >= 1 * eye(dimN);
